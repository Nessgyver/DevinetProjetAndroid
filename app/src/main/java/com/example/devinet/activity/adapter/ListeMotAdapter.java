...
public class ListeMotAdapter extends ArrayAdapter<Categorie> {

    private Context context;

    public ListeMotAdapter(@NonNull Context context, int resource, @NonNull List<Categorie> objects) {
    ...
    public View getView(int position, @Nullable View nouvelleLigne, @NonNull ViewGroup parent) {
        ...
        TextView tvListe = nouvelleLigne.findViewById(R.id.tv_ligne_niveau);
        final ProgressBar pbListe = nouvelleLigne.findViewById(R.id.pb_ligne_niveau);
        ...
        listeMotsCategorie.observe((LifecycleOwner) this.getContext(), new Observer<List<Mot>>() {
            ...
            public void onChanged(List<Mot> mots) {
                ...
                if (mots.size()>0){
                    ...
                    pbListe.setProgress((int)(motOk/mots.size()*100));
                }
            }
        });

        tvListe.setText("Liste" + (position + 1) );
